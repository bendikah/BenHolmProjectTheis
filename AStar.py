...
def AStarAlgorithm(start_node, end_node, grid, enc):
    ...
    closed_list = []
    start_node.g = 0
    start_node.f = start_node.g + heuristic(start_node, end_node)  # -math.inf
...
def is_valid_node(node, enc):
    if node.box_object.intersects(enc.land.geometry) \
    or node.box_object.intersects(enc.shore.geometry):
        print("Invalid node!")
        return False
    return True
...
def draw_line_path(start_point, end_point, path, enc):
    point_list = []
...
def neighborNodes(node_index, node_grid, enc, current_node):
    neighbors = []
    radial_cost = 0.9  # cost of moving radially
    ...
    if node_index[1] != 0:
        ...
        if west_box.intersects(enc.land.geometry) or west_box.intersects(enc.shore.geometry):
    ...
    try:
        ...
        if east_box.intersects(enc.land.geometry) or east_box.intersects(enc.shore.geometry):
    ...
    try:
        ...
        if north_box.intersects(enc.land.geometry) or north_box.intersects(enc.shore.geometry):
    ...
    try:
        ...
        if south_box.intersects(enc.land.geometry) or south_box.intersects(enc.shore.geometry):
    ...
    if node_index[0] != 0 and node_index[1] != 0:
        ...
        if south_west_box.intersects(enc.land.geometry) or south_west_box.intersects(enc.shore.geometry):
    ...
    try:
        ...
        if south_east_box.intersects(enc.land.geometry) or south_east_box.intersects(enc.shore.geometry):
    ...
    if node_index[0] != 0 and node_index[1] != 0:
        ...
        if north_west_box.intersects(enc.land.geometry) or north_west_box.intersects(enc.shore.geometry):
    ...
    try:
        ...
        if north_east_box.intersects(enc.land.geometry) or north_east_box.intersects(enc.shore.geometry):
